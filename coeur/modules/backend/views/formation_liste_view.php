...
<a href="<?php echo site_url('backend/formation/formation_ajout/')?>" class="btn btn-sm btn-primary" >Ajouter</a>
...
<br><br>
         <table class="table table-striped table-bordered table-vcenter js-dataTable-full-pagination">
                <thead>
                   ...
                   <?php if ($liste_formation != null): ?>
                        <?php foreach ($liste_formation as $row): ?>

                               <tr>

                                    <td>  <strong style="color: green"><?php echo strtoupper($row->type); ?></strong></td>
                                    <td class="font-w600"><?php echo $row->titre; ?></td>
                                    <td><?php if(!empty($row->fichier)) { echo "Oui" ;}?>                                 </td>
                                    <td class="font-w600"><?php echo $row->dates; ?></td>
                                    <td class="text-center">
                                        <div class="btn-group">
                                             <button class="btn btn-xs btn-default" type="button" data-toggle="tooltip" title="Editer" onClick="javascript:location.href='<?php echo site_url('backend/formation/formation_recup/'.$row->id) ; ?>'"><i class="fa fa-pencil"></i></button>



                                        <button class="btn btn-xs btn-default" type="button" data-toggle="tooltip" onClick="javascript:if(window.confirm('Voulez-vous supprimer ?')){location.href='<?php echo site_url('backend/formation/formation_supp/'.$row->id) ; ?>'; return true;} else {return false;}" title="Supprimer"><i class="fa fa-times"></i></button>
