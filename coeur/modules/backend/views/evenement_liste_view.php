            ...
            <main id="main-container">
                ...
                <div class="content bg-image overflow-hidden" style="background-image: url('<?php echo base_url() ?>assets/admin/img/photos/photo3@2x.jpg');">
                    <div class="push-50-t push-15">
                        ...
                        <h2 class="h5 text-white-op animated zoomIn" style="color: yellow;">                        EVENEMENTS
...
<a href="<?php echo site_url('backend/evenement/evenement_ajout/')?>" class="btn btn-sm btn-primary" >Ajouter</a>
...
<br><br>
         <table class="table table-striped table-bordered table-vcenter js-dataTable-full-pagination">
                <thead>
                   ...
                   <?php if ($liste_evenement != null): ?>
                        <?php $i = 1; ?>
                        <?php foreach ($liste_evenement as $row): ?>

                               <tr>

                                    <td class="font-w600"><?php echo $i; ?></td>
                                    <td class="font-w600" style="color: #00664C"><?php echo $row->type; ?></td>
                                    <td class="font-w600"><?php echo $row->titre_actu; ?></td>
                                    <td><?php if(!empty($row->fichier)) { echo "Oui" ;}?></td>
                                    <td><?php echo date('d/m/Y',strtotime($row->date_actu));?></td>
                                    <td class="text-center">
                                        <div class="btn-group">
                                             <button class="btn btn-xs btn-default" type="button" data-toggle="tooltip" title="Editer" onClick="javascript:location.href='<?php echo site_url('backend/evenement/evenement_recup/'.$row->id) ; ?>'"><i class="fa fa-pencil"></i></button>



                                        <button class="btn btn-xs btn-default" type="button" data-toggle="tooltip" onClick="javascript:if(window.confirm('Voulez-vous supprimer cet admin ?')){location.href='<?php echo site_url('backend/evenement/evenement_supp/'.$row->id) ; ?>'; return true;} else {return false;}" title="Supprimer"><i class="fa fa-times"></i></button>


                                            <!-- <button class="btn btn-xs btn-default" type="button" data-toggle="tooltip" title="Ajouter des photos" onClick="javascript:location.href='<?php //echo site_url('backend/evenement/photo_liste/'.$row->id.'/'.$this->uri->segment(3)) ; ?>'"><i class="fa fa-picture-o"></i></button>-->
