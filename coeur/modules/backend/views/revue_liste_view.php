            ...
            <main id="main-container">
                ...
                <div class="content bg-image overflow-hidden" style="background-image: url('<?php echo base_url() ?>assets/admin/img/photos/photo3@2x.jpg');">
                    <div class="push-50-t push-15">
                        ...
                        <h2 class="h5 text-white-op animated zoomIn" style="color: yellow;">                        REVUE DE PRESSE
...
<a href="<?php echo site_url('backend/revue/revue_ajout/')?>" class="btn btn-sm btn-primary" >Ajouter</a>
...
<br><br>
         <table class="table table-striped table-bordered table-vcenter js-dataTable-full-pagination">
                <thead>
                   ...
                   <?php if ($liste_revue != null): ?>
                        <?php $i = 1; ?>
                        <?php foreach ($liste_revue as $row): ?>
                               ...
                               <tr>
                                   ...
                                   <?php
                                    $nbre=$this->revue_model->sql_nombrephoto($row->id);
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group">
                                             <button class="btn btn-xs btn-default" type="button" data-toggle="tooltip" title="Editer" onClick="javascript:location.href='<?php echo site_url('backend/revue/revue_recup/'.$row->id) ; ?>'"><i class="fa fa-pencil"></i></button>

                                       <?php if($nbre==0) { //si aucun enregistrement de photo ?>

                                        <button class="btn btn-xs btn-default" type="button" data-toggle="tooltip" onClick="javascript:if(window.confirm('Voulez-vous supprimer ?')){location.href='<?php echo site_url('backend/revue/revue_supp/'.$row->id) ; ?>'; return true;} else {return false;}" title="Supprimer"><i class="fa fa-times"></i></button>
                                       <?php }?>

                                         <button class="btn btn-xs btn-default" type="button" data-toggle="tooltip" title="Ajouter des photos" onClick="javascript:location.href='<?php echo site_url('backend/revue/revuephoto_liste/'.$row->id) ; ?>'"><i class="fa fa-picture-o"></i></button>
